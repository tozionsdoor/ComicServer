...
if not UNRAR_AVAILABLE:
    _bundled_unrar = _resource_path("UnRAR.exe")
    if _bundled_unrar.exists():
        UNRAR_PATH = str(_bundled_unrar)
...
# 本文ページキャッシュの最大ファイル数（超過分は _trim_page_cache が削除）。
# 実測: 本ページ 1枚 約175KB、サムネイル(?w=) 1枚 約8KB。
# 10000 で概ね 1.7GB。旧型PCでも許容できる値としてここを既定にしている。
# 設定画面には出さない方針（「誰でも簡単に」がコンセプトで、容量の概念を
# 前提にした設定項目はユーザーを迷わせるだけのため）。
# なお「巻頭へ戻ると毎回遅い」問題の真因は上限ではなく削除順だったので、
# ここを大きくして誤魔化す必要はない → _trim_page_cache のコメント参照。
...
def _conn_type(ip: str) -> str:
    ...
    IPv6はNATが無いため、同じWi-Fi LAN上の端末でもグローバルに見えるアドレスを
    SLAACで持つ。サーバー自身のIPv6（get_global_ipv6）と同じ/64プレフィックスなら
    「外部直結」ではなく同一LANとして判定する（誤って毎回再接続トリガーが走る
    ことはないが、ログの接続種別表示が実態と合っていなかったため修正）。"""
...
def status(request: Request, _: str = Depends(_check_auth)):
    ...
    # Firebaseとは無関係なローカルログ（接続種別が変わった時だけ記録。スパム防止）。
...
_upnp_fresh_add_done: bool = False           # 起動直後の Delete→新規Add（ルーターの転送状態リセット）が済んだか
_http_conn_log: dict[str, str] = {}          # IP → 直前の接続種別（種別が変わった時だけログ）
...
def _upnp_open_ipv4(internal_ip: str, port: int) -> str:
    ...
    try:
        ...
        #
        #    ★起動直後のみ Delete→新規Add でルーターの転送状態を強制リセットする。
        #    PR-400NE は lease=0（永続）のマッピングを外部から見て登録済み（718を返す）でも、
        #    サーバー再起動でUPnP制御接続が切れた後に転送を内部で無効化する不具合がある。
        #    その場合 _add は 718 を返し「既存あり」と判断されるが実際には転送していない。
        #    起動時に1回 _del → _add することで常に「新規作成」状態を保証する。
        global _upnp_fresh_add_done
        if prev:
            if not _upnp_fresh_add_done:
                # 起動1回目: 古いエントリを削除して新規追加（転送状態を確実にリセット）
                _upnp_fresh_add_done = True
                try:
                    _del(prev)
                except Exception:
                    pass
            resp = _add(prev, 0)                 # 新規追加 or 再登録（永続リース優先）
            f = _upnp_fault(resp)
...
def _acquire_single_instance() -> bool:
    """True = 最初のインスタンス（起動継続）。
    False = 既存インスタンスに「表示」を送信済み → 呼び出し元はすぐ終了すべき。"""
    global _ipc_sock
    try:
...
def _start_ipc_listener(app: App) -> None:
    """IPC接続を待機し「show」を受け取ったらウィンドウを前面に出す。"""
    def _listen():
        while _ipc_sock:
            try:
                conn, _ = _ipc_sock.accept()
                try:
                    data = conn.recv(64)
                finally:
                    conn.close()
