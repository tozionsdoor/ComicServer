...
def load_config() -> dict:
    ...
    if not cfg.get("token"):
        ...
        changed = True
    # room_id はFirebase設定がある場合のみ自動生成（Firebase未設定でも手動でセット可能）
...
def _connection_info() -> dict:
    return {
        ...
        "room_id": _config.get("room_id", ""),  # WebRTC部屋ID（LANペアリングで端末へ渡す）
        "host":    get_local_ip(),
...
def _handle_dc_request(message: bytes | str) -> bytes:
    ...
    # 認証層2: room_idでP2Pが成立しても、トークン提示が無ければ本を配らない（多層防御）。
    token = _config.get("token", "")
    if not (token and supplied and secrets.compare_digest(supplied, token)):
