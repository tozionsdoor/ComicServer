...
def status(request: Request, _: str = Depends(_check_auth)):
    ip  = _client_ip(request)
    now = time.monotonic()
    if now - _http_conn_log.get(ip, 0) > 60:   # 1分に1回だけ記録（毎リクエストのスパム防止）
        _http_conn_log[ip] = now
        _log_queue.put(f"[HTTP] 接続: {ip} ({_conn_type(ip)})")
    return {
        "books": len(_books),
...
_http_conn_log: dict[str, float] = {}        # IP → 最終ログ時刻（接続種別を1分に1回だけログ）
